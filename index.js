...
app.post('/log_in',(request,response)=>{
        ...
        if(!email){
            ...
            pool.query('select * from user50 where email = ($1)',[email],(err,user)=>{
                if(!user.rowCount){
...
app.get('/allusers',Authenticate_Token,admin_authorization,async(request,response)=>{
                users = await pool.query('select * from user50')
